...
def ask_layer_name():
    def limit_text(*args):
        value = layer_name.get()
        if len(value) > 15:
            layer_name.set(value[:15])
    ...
    style.configure("Accent.TButton", font=("Segoe UI", 10, "bold"), padding=8,
                    foreground="white", background="#4CAF50", borderwidth=0)
    style.map("Accent.TButton", background=[("active", "#45a049")])

    style.configure("Cancel.TButton", font=("Segoe UI", 10, "bold"), padding=8,
                    foreground="white", background="#f44336", borderwidth=0)
    style.map("Cancel.TButton", background=[("active", "#d32f2f")])

    tk.Label(root, text="Layer Name:", font=("Segoe UI", 10), bg="#2b2b2b", fg="white").grid(row=0, column=0, padx=10, pady=10, sticky="e")

    layer_name = tk.StringVar(value="Layer")
    layer_name.trace_add("write", limit_text)

    ttk.Entry(root, textvariable=layer_name, font=("Segoe UI", 10)).grid(row=0, column=1, padx=10, pady=10)
